...
class WeightFactoryTest {
    ...
    void whenGramsGreaterThanZero_thenObjectCreated(long amount) {
        var weight = new Weight(BigDecimal.valueOf(amount));

        assertNotNull(weight);
        assertThat(weight.weightGrams()).isEqualByComparingTo(BigDecimal.valueOf(amount));
    }
    ...
    void whenGramsLessThanZero_thenThrowException(long amount) {
        assertThatThrownBy(() -> new Weight(BigDecimal.valueOf(amount)))
